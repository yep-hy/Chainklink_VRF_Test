...
function getPayLoad(contractABI, functionName, param) {
    for (let i = 0; i < contractABI.length; i++) {
        ...
        //payload
        const payload =
            functionSigHash + codeOfParam.substring(2, codeOfParam.length);
...
!developmentChains.includes(network.name)
    ...
    : describe("MultiSigWallet test", function () {
          it("test function of MultiSigWallet.sol", async function () {
              ...
              const MultiSigWalletContractFactory =
                  await ethers.getContractFactory("MultiSigWallet");
              const multiSigWalletContract =
                  await MultiSigWalletContractFactory.deploy(
              ...
              const HelloContractFactory = await ethers.getContractFactory(
                  "Hello"
              );
              ...
              // 检查结果（不通过） 当前有1人确认
              expect((await helloContract.balance()).toString()).not.to.equal(
                  "233"
              );

              //当前交易数量是 1
              const transactionCount =
                  await multiSigWalletContract.transactionCount();
              console.log("transaction count: ", transactionCount);
              expect(transactionCount.toString()).to.equal("1");

              const transactionId = await transactionReceipt.events[0].args
                  .transactionId;
              console.log("transaction id: ", transactionId);

              //当前已确认人数是 1
              const confirmations1 =
                  await multiSigWalletContract.getConfirmations(transactionId);
              console.log("confirmations number: ", confirmations1.length);
              expect(confirmations1.length.toString()).to.equal("1");

              const isConfirmed1 = await multiSigWalletContract.isConfirmed(
                  transactionId
              );
              ...
              //当前已确认人数是 2
              const confirmations2 = await multiSigWalletContract
                  .connect(Bob)
              ...
              const isConfirmed2 = await multiSigWalletContract
                  .connect(Bob)
              ...
              // 再次检查结果（通过） 当前有2人确认
              expect((await helloContract.balance()).toString()).to.equal(
                  "233"
              );
          });
      });
